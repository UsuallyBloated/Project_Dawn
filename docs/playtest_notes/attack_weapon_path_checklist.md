...
- [x] Have a melee character with a weapon to equip (via `/give` + paperdoll if needed) and an enemy

## 1 — Equipped melee weapon (regression)
- [x] **Equip a 1H weapon and auto-attack an enemy** → normal damage lands, weapon skill advances as
  before. notes:
- [x] **Swap to a different (heavier/lighter) weapon and attack** → damage tracks the NEW weapon (the
  server re-derives from the equipment slot). notes:

## 2 — Unarmed (regression)
- [x] **Unequip your weapon and attack bare-handed** → you still hit for fist damage (1-4 + STR
  bonus), hand_to_hand skill advances. notes:

## 3 — Dual-wield (regression)
- [x] **Equip a main-hand + off-hand weapon and attack** → both swings land; off-hand does its
  reduced damage; each trains its own weapon skill. notes:
- [x] **Remove the off-hand only, keep main-hand, attack** → main-hand still swings normally; no
  phantom off-hand hit. notes:

## 4 — Ranged (regression)
- [x] **Equip a bow and attack from range** → the longer ranged envelope still applies (no false
  "out of range" Miss at bow distance). notes:

## 5 — Range enforcement still correct
- [x] **With a melee weapon, try to hit an enemy that's clearly too far** → Miss (out of range), same
...
- [-] **Send an Attack naming a weapon you don't have equipped** (needs a modified client) → server
  ...
  notes: (expected `[-]` — stock client always sends its real weapon; unit-test-covered) as you expected, i did not test this with the stick client.

## Notes / observations
- Biggest thing i noticed combat skills reset to 1/x when i re-login.  however when character's combat skills increase the correct numbers appear and replace the 1/x.  Let me know if this makes sense.  Appears to be persistent on server side but skills dont load when character logs in.
