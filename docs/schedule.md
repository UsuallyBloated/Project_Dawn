...
| 1. Exploit gate | Jul 20 to Jul 31 | **In progress** — keystone (`is_gm`) + Respawn dead-check + attack-while-seated + cast class/level gate + `Attack` weapon_path + melee swing-rate limit all done & playtested (through 07-29, server `335b5b1`). Remaining: login rate-limit + auth timing |
