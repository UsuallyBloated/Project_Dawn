...
| **`world_two_clients.rs` integration tests are load-sensitive** | Noise in any server change | Known, documented in `server/docs/flaky_integration_tests.md`. The rule (corrected twice): a failure that reproduces in isolation is real; one that passes alone is load-sensitivity — and the clean-tree A/B is the final arbiter. |
