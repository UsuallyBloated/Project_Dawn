# World — Zone Index

The world of Project Dawn is divided into named zones. Each zone is a discrete area with its own ecology, faction presence, enemy types, and level range. Zones connect to each other via zone transitions (load screens, portals, or geographic passages).

## Zones

| Zone | Level Range | Type | Primary Factions | Notes |
|---|---|---|---|---|
| [Greenshire](greenshire.md) | 1–10 | Starting area | Human, Halfling | Gentle farmlands; newbie content; first hub town |
| [Ashfang Hold](ashfang_hold.md) | 1–10 | Evil starting area | Ogre, Troll, Half-Ogre | Warlord proving camp; evil-aligned newbie content |
| [The Bonecourt](bonecourt.md) | 1–10 | Evil starting area | Kel`varath | Bonewright necromantic outpost; jungle ruins; Kel`varath start |
| [The Kobold Deeps](kobold_deeps.md) | 1–10 | Evil starting area | Kobold | Pack tunnel network beneath Cogsworth; Kobold start |
| [Ardenmoor](ardenmoor.md) | 5–15 | Forest / Wilderness | Ranger guilds, Wood Elf | Dense woodland; nature content; early wilderness experience |
| [The Greyfen](greyfen.md) | 10–20 | Fen / Predecessor site | Cartography Hall, Walking Name Order | Fen corridor between Ardenmoor and Greyveil; Foundation Ring edges surface here; Peat Hall dungeon |
| [Aelindra](aelindra.md) | 10–20 | Elf city / Forest | High Elf, Arcane schools | Elven city with two dungeons; Dreaming Grove at root level; Sealed Archive; caster hub |
| [Cogsworth](cogsworth.md) | 10–25 | City / Underground | Gnome, Kobold | Mechanical city; Tinkering hub; multi-level industrial zone |
| [Dwarven Mountain Holds](dwarven_holds.md) | 15–25 | Underground / Dungeon | Dwarf clans | Forge culture; Runeforging center; deep mine dungeons |
| [Harrowmere](harrowmere.md) | 15–30 | Coastal port city | Dockmasters' Compact | Independent neutral port; maritime content; sea dungeon |
| [The Drowned Hold](drowned_hold.md) | 20–30 | Maritime dungeon / Architect node | Dominion garrison (undead), Dockmasters' Compact | Dominion naval installation on Architect seamount in Harrow Bay; six levels; 300-year-old garrison; confirmed network node |
| [Greyveil](greyveil.md) | 20–30 | Human city | Merchants, Rogues, Bards | Urban intrigue zone; large neutral city; Bard colleges |
| [Auroch's Rest](aurochs_rest.md) | 20–30 | Plains / Minotaur lands | Minotaur clans | Open plains; Minotaur cultural center; Shaman tradition |
| [The Blighted Wood](blighted_wood.md) | 20–30 | Corrupted forest | Corrupted Grove, Wood Elf rangers | Dying eastern Ardenmoor; horror content; vertical tree dungeon |
| [The Ashen Coast](ashen_coast.md) | 20–35 | Volcanic shoreline | Cinder Docks (independent) | Fire elemental content; Magician nexus sites; volcano dungeon |
| [Fae Mere](fae_mere.md) | 25–35 | Fae court / Forest | Fae courts | Dream-logic zone; illusion-heavy; Enchanter and Fae hub |
| [Varek's Spire](wizard_tower.md) | 25–35 | Wizard tower / Dungeon | Neutral (Varek) | 7-floor tower + basement; Wizard trainer; The Residual mystery |
| [Eastern Badlands](eastern_badlands.md) | 25–40 | Wasteland | Ogre, Troll, Half-Ogre | Harsh open territory; Evil-neutral content; named mob hunting |
| [The Underdark Crossing](underdark_crossing.md) | 25–40 | Underground transit | Kobold packs, Dark Elf patrols | Contested tunnel network; Grey Market; third digger mystery |
| [The Breach](siege_location.md) | 25–40 | Contested siege zone | Vanguard (Good), Iron Camp (Evil) | 6-hour control windows; PvP/PvE; Gate-Key one per server |
| [Southeastern Marshlands](southeastern_marshlands.md) | 30–40 | Swamp / Dungeon | Kel`varath tribes, undead | Reptilian culture; Monk discipline centers; undead at night |
| [The Ashen Marches](revenant_territory.md) | 30–45 | Undead / Transformation zone | Dominion Remnant, Coalition Remnant, Third Army | Three-faction revenant war; transformation site; server-scale dungeon event |
| [Khala Savannahs](khala_savannahs.md) | 30–45 | Savannah / Grassland | Felhari clans | Felhari homeland; Spirit-Walker tradition; wide open terrain |
| [Ixareth-Kul — Ruined Dominion](ruined_dominion.md) | 35–50 | Open world dungeon | Bonewrights, Soulcarvers, Ash Scribes | Fallen Kel`varath capital; three-faction conflict; Dominion Vault endgame |
| [Vel'Sharath](vel_sharath.md) | 35–50 | Dark Elf city / Underground | Dark Elf, Necromancer guilds | Evil-aligned city; dangerous for Good characters; deepest Evil content |
| [The Dominion Vault](dominion_vault.md) | 50+ | Endgame dungeon / Architect terminus | Three Dominion Orders | Sealed archive beneath Ixareth-Kul; Architect survey terminus; Gate-Key required; server-scale event |
| [The Rising Shell](rising_shell.md) | 50+ | Endgame dungeon / Architect anchor node | Walking Name Order | Architect terminal node rising from Cinder Deep; post-Calibration State access; consequence of the server-scale event manifests here |

## Zone Design Principles

**Level range is a guideline, not a hard gate.** Solo characters at the low end of a range struggle; groups near the high end find the content easy. The overlap between zones is intentional — players move when they are ready, not when a number tells them to.

**Faction matters at zone level.** Good-aligned characters face hostility in Vel'Sharath. Evil-aligned characters are unwelcome in Aelindra or the Dwarven Holds. Greyveil is the major neutral city — almost anyone can operate there, which makes it the trade and social hub of the mid-game.

**Each zone has at least one dungeon.** The overworld content of each zone provides leveling experience. The dungeon inside or adjacent provides the group content, named enemies, and best loot for that zone's tier.

**Zones have day/night ecology.** Undead content spikes at night. Certain rare spawns only appear at specific times. Safe resting spots matter because the world changes around you.
